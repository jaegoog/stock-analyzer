...
| 항목 | 상태 |
|------|------|
| 전용 API 라우트 | **없음** — `/api/stocks/...` 어디에도 리포트 수집 엔드포인트가 없다. |
| `GET .../news` | **언론·기사** (KR: Naver Finance, US: Finnhub). 증권사 PDF·목표가·투자의견 요약 등 **리서치 리포트와는 별개**. |
| Brick `research` | manifest의 **UI 카테고리**(`외부 데이터 조회`)일 뿐, 증권사 리포트 전용 기능을 뜻하지 않는다. |

추가 시에는 **공식 API·유료 데이터 피드·명시적 라이선스**가 있는 소스를 우선하고, 새 Provider 또는 Brick + `/api/...` 라우트로 일관되게 확장한다.

### 스크래핑
| 항목 | 내용 |
|------|------|
| 기술적 | HTTP + HTML 파싱 등으로 **구현 가능**. |
| 법·약관 | 사이트별 이용약관, `robots.txt`, **저작권**, 로그인·유료 영역은 **사전 검토 필요** (법률 자문 아님). |
| 본 프로젝트 사례 | KR **뉴스**는 Provider `naver-finance`가 서버 측에서 HTML을 파싱해 공급한다 (`GET /api/stocks/[ticker]/news`, 시장 KR). |
| 운영 | 마크업 변경에 취약하므로 장기적으로는 **공식 API**가 유리하다. |
